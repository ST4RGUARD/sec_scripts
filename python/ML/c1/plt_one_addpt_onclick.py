...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        try:
            # First try the newer matplotlib version attributes
            if hasattr(bcid, 'rectangles'):
                rect = bcid.rectangles[0]
                h = rect.get_height()
                rect.set_height(3 * h)
            # Then try older matplotlib version attributes
            elif hasattr(bcid, '_boxes'):
                rect = bcid._boxes[0]
                h = rect.get_height()
                rect.set_height(3 * h)
            else:
                # If neither attribute exists, silently return
                return
            ...
            # Update lines with compatibility check
            if hasattr(bcid, 'lines'):
                bcid.lines[0][0].set_ydata([ymax, ymin])
                bcid.lines[0][1].set_ydata([ymin, ymax])
            elif hasattr(bcid, '_lines'):
                bcid._lines[0][0].set_ydata([ymax, ymin])
                bcid._lines[0][1].set_ydata([ymin, ymax])
        except Exception:
            # If any error occurs, silently continue
            # This prevents the visualization from breaking if the widget structure changes
            pass